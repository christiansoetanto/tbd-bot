...
func GenerateCalendarEmbed() (*discordgo.MessageEmbed, bool, error) {
	...
	text, isAnyHDO, isAnyFeastOrSolemnity := getCelebrations(allLiturgicalDays.LiturgicalDaysEn)
	...
	isMentionLatinCath := isAnyHDO || isAnyFeastOrSolemnity && currentTime.Weekday() != time.Sunday
	embed := EmbedBuilder(title, text, fields)
	return embed, isMentionLatinCath, nil
}
func getCelebrations(liturgicalDays []LiturgicalDay) (string, bool, bool) {
	text := "The Roman Catholic Church is celebrating:\n"
	isAnyHDO := false
	isAnyFeastOrSolemnity := false
	...
	return text, isAnyHDO, isAnyFeastOrSolemnity
